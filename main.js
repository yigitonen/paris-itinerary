...
function renderStop(stop, index, trip, day) {
  ...
  return `<div class="stop-item"><time class="stop-time">${escapeHtml(stop.time || '—')}</time><span class="stop-dot">${index + 1}</span><article class="stop-card"><div class="stop-content">${travel}<div class="stop-badges">${importance ? `<span class="stop-badge importance-${escapeHtml(stop.importance)}">${escapeHtml(importance)}</span>` : ''}${stop.verified ? '<span class="stop-badge verified-badge"><i data-lucide="badge-check"></i>Maps kaynağıyla doğrulandı</span>' : ''}${rating}</div><h3>${escapeHtml(stop.title)}</h3>${mapsAttribution}<p class="stop-note">${escapeHtml(stop.notes || stop.address || 'Not eklenmedi.')}</p>${stop.why ? `<p class="stop-insight"><strong>Neden burada?</strong>${escapeHtml(stop.why)}</p>` : ''}${stop.travelerNote ? `<p class="stop-insight traveler-insight"><strong>Gezginlerden ortak not</strong>${escapeHtml(stop.travelerNote)}</p>` : ''}<small>${escapeHtml(stop.category || 'Durak')}${stop.duration ? ` · ${escapeHtml(stop.duration)}` : ''}${stop.address ? ` · ${escapeHtml(stop.address)}` : ''}</small></div><div class="stop-tools"><a href="${stopMapsUrl(stop, trip)}" target="_blank" rel="noopener" aria-label="Google Maps'te aç"><i data-lucide="navigation"></i></a><button data-edit-stop="${stop.id}" data-day-id="${day.id}" aria-label="Durağı düzenle"><i data-lucide="pencil"></i></button></div></article></div>`;
}
